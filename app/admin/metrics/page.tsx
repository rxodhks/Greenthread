    ...
    return (
        <div className="min-h-screen bg-emerald-950 px-4 py-10 text-emerald-50 sm:px-6">
            <div className="mx-auto max-w-3xl">
                ...
                {error && (
                    <p
                        className="mt-4 rounded-lg border border-red-400/40 bg-red-950/50 px-3 py-2 text-sm text-red-100"
                        role="alert"
                    >
                        {error}
                    </p>
                ...
                {data && (
                    <div className="mt-8 space-y-6">
                        <div className="rounded-xl border border-emerald-500/20 bg-emerald-900/40 p-4 text-sm">
                            <p>
                                저장소: <strong>{data.storage}</strong> · 샘플{" "}
                                {data.sampleSize}건
                            </p>
                            {data.oldestTs && data.newestTs && (
                                <p className="mt-1 text-emerald-100/70">
                                    기간:{" "}
                                    {new Date(data.oldestTs).toLocaleString(
                                        "ko-KR",
                                    )}{" "}
                                    —{" "}
                                    {new Date(data.newestTs).toLocaleString(
                                        "ko-KR",
                                    )}
                                </p>
                            )}
                            {data.storage === "memory" && (
                                <p className="mt-3 rounded-lg border border-amber-500/30 bg-amber-950/40 p-3 text-xs leading-relaxed text-amber-100/90">
                                    서버가 Redis를 읽지 못한 상태입니다. Vercel → Project →{" "}
                                    ...
                                    가 있는지 확인한 뒤, Storage에서 이 프로젝트에 DB가 연결됐는지 확인하고{" "}
                        ...
                        <div className="rounded-xl border border-emerald-400/30 bg-emerald-900/60 p-5">
                            ...
                            <p className="mt-1 text-sm text-emerald-100/80">
                                {data.hypothesis.note}
                            </p>
                            <p className="mt-4 text-3xl font-semibold tabular-nums text-white">
                                {data.aggregate.ctaClickRatePct != null
                                    ? `${data.aggregate.ctaClickRatePct}%`
                                    : "—"}
                            </p>
                            <p className="mt-1 text-sm text-emerald-200/80">
                                목표(가설): 약{" "}
                                {data.hypothesis.targetCtaRatePct}% · 스캔 성공{" "}
                                {data.aggregate.scanSuccess} · CTA 클릭{" "}
                                {data.aggregate.ctaDonate +
                                    data.aggregate.ctaAlternatives}{" "}
                                (기부 {data.aggregate.ctaDonate} / 대체{" "}
                                {data.aggregate.ctaAlternatives})
                            </p>
                        </div>
                        ...
                        <div className="grid gap-4 sm:grid-cols-2">
                            <div className="rounded-xl border border-emerald-500/20 bg-emerald-900/40 p-4">
                                ...
                                <p className="mt-2 tabular-nums">
                                    성공 {data.aggregate.scanSuccess}
                                </p>
                                <p className="tabular-nums text-emerald-200/80">
                                    실패 {data.aggregate.scanError}
                                </p>
                                <p className="mt-2 text-xs text-emerald-300/70">
                                    비전 {data.aggregate.byScanSource.vision} ·
                                    데모 {data.aggregate.byScanSource.mock}
                                </p>
                            </div>
                            <div className="rounded-xl border border-emerald-500/20 bg-emerald-900/40 p-4">
                                <h3 className="text-sm font-medium text-emerald-100">
                                ...
                                <p className="mt-2 tabular-nums">
                                    share_click {data.aggregate.shareClick}
                                </p>
                                <p className="tabular-nums text-emerald-200/80">
                                    복사 성공 {data.aggregate.shareCopySuccess}{" "}
                                    · 네이티브 {data.aggregate.shareNative}
                                </p>
                            </div>
                        </div>
                            ...
                            <ul className="mt-2 max-h-48 overflow-auto text-sm tabular-nums text-emerald-100/90">
                                {Object.entries(data.aggregate.byEvent)
                                    ...
                                    .map(([k, v]) => (
                                        <li
                                            key={k}
                                            className="flex justify-between gap-4 border-b border-emerald-800/40 py-1"
                                        >
                                            <span>{k}</span>
                                            <span>{v}</span>
                                        </li>
